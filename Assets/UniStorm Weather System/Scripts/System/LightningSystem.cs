...
namespace UniStorm.Utility
{
    public class LightningSystem : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            LightningBolt.transform.SetParent(FindAnyObjectByType<UniStormSystem>().transform);
            ...
            EndingPoint.transform.SetParent(FindAnyObjectByType<UniStormSystem>().transform);
            EndingPoint.name = "Lightning End Point";

            StartingPoint = new GameObject().transform;
            StartingPoint.transform.position = Vector3.zero;
            StartingPoint.SetParent(FindAnyObjectByType<UniStormSystem>().transform);
            ...
            AS.outputAudioMixerGroup = FindAnyObjectByType<UniStormSystem>().UniStormAudioMixer.FindMatchingGroups("Master/Weather")[0];
        ...
        IEnumerator DrawLightning()
        {
            ...
            if (UniStormSystem.Instance.LightningStrikes == UniStormSystem.EnableFeature.Enabled)
            {
                ...
                if (EndingPoint.GetComponent<LightningStrike>().EmeraldAIAgentDetected)
                {
                    if (EndingPoint.GetComponent<LightningStrike>().HitAgent != null)
                    {
                ...
                while (Generated)
                {
                    ...
                    TempColor2.a = Mathf.Lerp(1, UniStormSystem.Instance.m_LightningLight.intensity / 1.75f, m_FlashTimer);
